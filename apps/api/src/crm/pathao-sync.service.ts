...
@Injectable()
export class PathaoSyncService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(PathaoSyncService.name);
  ...
  private async tick() {
    ...
    this.running = true;
    try {
      const orgs = await this.integrations.listEnabledPathaoOrgs();
      const now = Date.now();
      for (const org of orgs) {
        ...
        try {
          ...
          await this.integrations.markSyncResult(org.organizationId, { ok: true });
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
          this.logger.warn(`Pathao sync failed for ${org.organizationId}: ${message}`);
          await this.integrations.markSyncResult(org.organizationId, {
            ok: false,
            error: message,
          });
        }
  ...
  async syncOrder(
    ...
    if (!order?.courierConsignmentId || order.courierProvider !== 'pathao') {
  ...
  private async syncOne(
    ...
    let info: Awaited<ReturnType<PathaoCourierService['getOrderInfo']>>;
    try {
      info = await this.pathao.getOrderInfo(
        organizationId,
      ...
      const message = err instanceof Error ? err.message : String(err);
      this.logger.warn(
        `Pathao sync skipped for ${order.orderNumber}: ${message}`,
      );
      // Avoid hammering the same missing/invalid consignment every tick.
    ...
    const raw = info.orderStatusSlug || info.orderStatus;
