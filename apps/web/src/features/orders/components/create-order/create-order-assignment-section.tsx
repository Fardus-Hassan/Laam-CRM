...
export function CreateOrderAssignmentSection({ form }: CreateOrderAssignmentSectionProps) {
  ...
  const creatorLabel = user?.name?.trim() || user?.email?.trim() || 'you';
  const defaultHint = `Assigns to ${creatorLabel} for KPI credit. Website orders use Settings → Assignment routing instead.`;

  React.useEffect(() => {
    let cancelled = false;
    void Promise.all([rbacApi.listTeams(''), rbacApi.listUsers('')])
      .then(([orgTeams, orgUsers]) => {
        if (cancelled) return;
        setTeams(orgTeams);
    ...
    <Card className="gap-0 py-0 shadow-none">
      ...
      <CardContent className={cn('grid', ORDER_SECTION_BODY_CLASS, ORDER_SECTION_GRID_GAP)}>
        <p className="col-span-full text-xs text-muted-foreground">
          ...
          Website ingest uses Settings → Assignment routing; manual CRM orders default to you.
        </p>
